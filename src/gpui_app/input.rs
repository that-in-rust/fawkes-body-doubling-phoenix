...
use gpui::{
    ...
    fill, hsla, point, prelude::*, px, relative, rgb, rgba,
};
...
impl SingleLineInput {
    pub fn new(
        ...
        let initial_length = initial_value.len();
        Self {
            focus_handle: cx.focus_handle(),
            content: initial_value,
            placeholder: placeholder.into(),
            selected_range: initial_length..initial_length,
    ...
    pub fn set_disabled_state(&mut self, is_disabled: bool) {
    ...
    fn guard_editing(&self) -> bool {
        !self.is_disabled
    }

    fn left(&mut self, _: &Left, _: &mut Window, cx: &mut Context<Self>) {
        if !self.guard_editing() {
            return;
        }
    ...
    fn on_mouse_move(&mut self, event: &MouseMoveEvent, _: &mut Window, cx: &mut Context<Self>) {
        if self.is_selecting && self.guard_editing() {
            self.select_to(self.index_for_mouse_position(event.position), cx);
        }
    }
    ...
    fn cut(&mut self, _: &Cut, window: &mut Window, cx: &mut Context<Self>) {
        if !self.guard_editing() || self.selected_range.is_empty() {
            return;
        }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let runs = if let Some(marked_range) = input.marked_range.as_ref() {
            ...
            .into_iter()
            .filter(|candidate| candidate.len > 0)
            .collect()
        } else {
            vec![run]
        };
    ...
    fn paint(
        ...
        if !self.input.read(cx).is_disabled {
            window.handle_input(
                &focus_handle,
...
impl Render for SingleLineInput {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let background_color = if self.is_disabled {
            rgb(0xf1f3f6)
        } else {
            rgb(0xffffff)
        };
        let cursor_style = if self.is_disabled {
            CursorStyle::Arrow
        } else {
            CursorStyle::IBeam
        };
